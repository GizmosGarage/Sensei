...
- a unique `error_id` shown with user-facing dashboard failures;
...
```powershell
python -m sensei --error-log D:\logs\study-errors.jsonl
python -m sensei.dashboard --error-log D:\logs\study-errors.jsonl
```

The active file rotates at 5 MiB. Ten archives are retained beside it as
`study-errors.jsonl.1` through `study-errors.jsonl.10`, with `.1` being the newest.
...
```powershell
Get-Content data\logs\study-errors.jsonl -Tail 20 |
