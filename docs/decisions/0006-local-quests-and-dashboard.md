...
Build the progress interface as packaged HTML, CSS, and JavaScript served by a Python standard-library HTTP server fixed to `127.0.0.1`. Read the same SQLite database through an answer-key-free GET API. Do not add browser-owned progression state or hosted persistence. ADR 0010 later adds server-side hosted model access while keeping credentials out of the browser.
...
- The dashboard keeps study history local; tutoring prompts cross only the configured provider boundary.
...
Rejected for the learning application because current study history is intentionally machine-local. A separate public demo with synthetic data can be considered later without changing this boundary.
