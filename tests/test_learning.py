...
from sensei.learning import (  # noqa: E402
    LearningEventError as _LearningEventError,
    MisconceptionClassifier,
    parse_misconception_finding,
)
from sensei.providers import CompletionResult as _CompletionResult  # noqa: E402
...
class MisconceptionClassifierTests(unittest.TestCase):
    ...
    def test_invalid_output_is_repaired_once_then_fails(self) -> None:
        ...
        with self.assertRaisesRegex(_LearningEventError, "did not return a valid misconception"):
            MisconceptionClassifier(failing).classify(**self.ARGUMENTS)
        with self.assertRaisesRegex(_LearningEventError, "confidence must be"):
            parse_misconception_finding(
